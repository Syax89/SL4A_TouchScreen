...
| Experimental standard-mode recovery (issue #4) | `std_liveness_ms`, `std_liveness_recover`, `skip_std_getfeat` | Off by default, never set by the installer. `std_liveness_ms` only logs; `std_liveness_recover` additionally runs the existing ACPI recovery when no input data arrives after `DONE`; `skip_std_getfeat` answers feature reads with `-EOPNOTSUPP` in standard mode so nothing is written to SPI for a feature query. Field-tested before any of these become defaults. |
