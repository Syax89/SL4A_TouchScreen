...
- [x] Publish versioned GitHub releases with source checksums (`v1.4.0`:
      source tarball + SHA256SUMS attached; module signing is DKMS's own
      per-machine MOK key generated at install time, not a release-time
      signature — see docs/ROLLBACK.md).
- [ ] Record a release-qualified (E1) compatibility table row for a tagged
      release. `docs/COMPATIBILITY.md` still has none; this release fixed
      real bugs in `activate` found by exercising it live on real hardware,
      but that was ad hoc verification, not the full matrix in
      `docs/TESTING.md`.
...
| 2026-07-24 | Consolidated `tools/{install,uninstall,activate-fch,rebuild_and_install}.sh` into one `tools/sl4a-touch.sh` (install/uninstall/activate/status/logs/rebuild), per direct user direction after live use surfaced real usability and correctness problems: `install` now auto-activates immediately and installs a `multi-user.target`-gated systemd unit so it also auto-activates on every future boot (closing the original manual-activation gap without reintroducing the early-boot-alias failure mode the black-screen fix addressed); running the tool with no arguments shows an arrow-key menu instead of requiring a memorized subcommand. Found and fixed four real bugs (broken Secure Boot/MOK enrollment check, an overly broad "no other MSHW\* device" check that unconditionally blocked activation on real Surface hardware, a stdout-leaking menu redraw, and an off-by-one in the menu's cursor math) only by actually running `activate` against this machine's live installed state — none of the day's earlier testing had exercised that path end to end. Separately fixed all three GitHub Actions CI jobs, failing since at least the `v1.3.0` tag: a kernel-version-dependent `platform_driver.remove` signature mismatch and a missing `linux/unaligned.h` on pre-6.11-ish kernels (both in the driver, `LINUX_VERSION_CODE`/`__has_include`-gated), a link-order bug dropping `-lm` under `--as-needed` linkers, and a whitespace check flagging ~34 files of verbatim forensic capture/decompile data (excluded those paths; fixed the handful of real hits). Tagged and published `v1.4.0` (source tarball + SHA256SUMS on the GitHub release). | E3 (installer/CI fixes, host-verified + live-hardware-verified for `activate`); E1 (standard HID `activate` end-to-end confirmed working on this Surface Laptop 4, single load/unload cycle) | `make test` and `make SANITIZE=1 test` green; clean `make LLVM=1`; all 3 CI jobs green on `main` after each fix, re-verified via `gh run watch`; real `sudo ./tools/sl4a-touch.sh activate` confirmed binding + a working HID input device on this machine. Raw multitouch profile not re-activated/re-tested with real fingers this session. |
