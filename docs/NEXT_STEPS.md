...
The device supports multi-touch in **raw heatmap mode**. The captured Windows exchange is
GET_FEATURE type `0x04`, feature ID `0x06`, followed by SET_FEATURE type `0x03`, feature
ID `0x05`, value `0x01`. Earlier references to a configurable selector `4` or `5` were a
field-position error and must not be used. Raw mode sends content_id=0x0C frames (~4302 bytes).
...
cannot explain why the captured SET_FEATURE attempt can silence the controller.
