...
[![Release](https://img.shields.io/badge/release-1.6.0-brightgreen)](VERSION)
...
| No touch after cold boot and no `RESET_RSP` in dmesg at all | Enable the backstop: `echo 'options sl4a_spi_hid wait_reset_kick_ms=4000' \| sudo tee /etc/modprobe.d/sl4a-kick.conf`, then cold boot. dmesg then shows `no RESET_RSP and no IRQ at all after 4000 ms, forcing DESCREQ`; if it instead shows `DESCREQ write to a silent controller failed 3 times`, the controller answered neither a reset nor a direct descriptor request: power off, unplug AC, wait 30 s, reboot and report the log in issue #4 |
