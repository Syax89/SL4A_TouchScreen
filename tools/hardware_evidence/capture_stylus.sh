#!/usr/bin/env bash
# Record a bounded, read-only evtest trace from the standard HID stylus node.
...
usage() {
	cat <<'EOF'
Usage: tools/hardware_evidence/capture_stylus.sh --duration SECONDS --output FILE

Discovers the SL4A standard HID stylus event node from sysfs identity, name, and
pen capabilities, then records a bounded evtest trace. FILE must not already
exist. The helper never invokes sudo or installs, loads, unloads, or configures
drivers.
EOF
}
...
for input in /sys/class/input/input*; do
	...
	[[ "$name" = *Stylus* ]] || continue
	capability_bit_set "$input/capabilities/ev" 3 || continue       # EV_ABS
	capability_bit_set "$input/capabilities/abs" 0 || continue      # ABS_X
	capability_bit_set "$input/capabilities/abs" 1 || continue      # ABS_Y
	capability_bit_set "$input/capabilities/key" 320 || continue    # BTN_TOOL_PEN
...
if [ "${#matches[@]}" -ne 1 ]; then
	printf 'Expected one SL4A standard HID stylus event node; found %s.\n' "${#matches[@]}" >&2
...
umask 077
{
	printf '# SL4A standard HID stylus evtest capture\n'
